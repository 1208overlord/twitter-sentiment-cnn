...
def load_data_and_labels(dataset_fraction):
    ...
    positive_examples = list(open(POS_DATASET_PATH, encoding="utf8").readlines())
    positive_examples = [s.strip() for s in positive_examples]
    print ("\tdata_helpers: [OK]")
    print ("\tdata_helpers: loading negative examples...")
    negative_examples = list(open(NEG_DATASET_PATH, encoding="utf8").readlines())
...
def build_vocab():
    ...
    """
    voc = csv.reader(open(VOC_PATH, encoding="utf8"))
    voc_inv = csv.reader(open(VOC_INV_PATH, encoding="utf8"))
    ...
    # with open(VOC_PATH, 'r') as read_obj:
    ...
    for x in voc:
        if not x:
            continue
        vocabulary[x[0]] = x[1]
    # vocabulary = {x[0]: x[1] for x in voc}
...
def batch_iter(data, batch_size, num_epochs):
    ...
    data_2 = list(data_1)
    data = np.array(data)
    data_size = len(data_2)
    num_batches_per_epoch = int(data_size/batch_size) + 1
    for epoch in range(num_epochs):
        # Shuffle the data at each epoch
        shuffle_indices = np.random.permutation(np.arange(data_size))
        shuffled_data = [data_2[idx] for idx in shuffle_indices.tolist()]
        # shuffled_data = data_2[shuffle_indices]
