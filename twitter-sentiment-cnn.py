...
def evaluate_sentence(sentence, vocabulary):
    ...
    factor = (unnorm_result[0][1] - unnorm_result[0][0]) * 10
    log('Factor: ', str(factor))
...
tf.flags.DEFINE_string('custom_input', 'I love neural networks!',
...
x_train, x_test = np.array(x_shuffled[:-test_index]), np.array(x_shuffled[-test_index:])
...
validate_every = len(y_train) / (FLAGS.batch_size * FLAGS.valid_freq) - 1
checkpoint_every = len(y_train) / (FLAGS.batch_size * FLAGS.checkpoint_freq) - 1
...
with tf.device(device):
    ...
    num_filters_total = FLAGS.num_filters * ii
...
if FLAGS.train:
    ...
    test_batches = batch_iter(zip(x_test, y_test), FLAGS.batch_size, 1)
    ...
    ##################################################
    batches_in_epoch_test = len(y_test) / FLAGS.batch_size
    batches_in_epoch_test = batches_in_epoch_test if batches_in_epoch_test != 0 else 1
    total_num_step_test = batches_in_epoch_test

    batches_progressbar_test = tqdm(test_batches, total=total_num_step_test,
                               desc='Starting evaluating...')
    ##################################################
    for batch in batches_progressbar:
        ...
        feed_dict = {data_in: np.array(x_batch),
                     data_out: np.array(y_batch),
        ...
        validate_every = 33290
        if global_step % validate_every == 0:
            accuracies = []
            losses = []
            # for test_batch in test_batches:
            for test_batch in batches_progressbar_test:
                if len(test_batch) == 0:
                    break
